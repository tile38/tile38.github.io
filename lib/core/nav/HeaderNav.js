...
class HeaderNav extends React.Component {
  ...
  makeLinks(link) {
    ...
    const itemClasses = classNames({
      siteNavGroupActive: !link.button // melbania: control header-button style
        ? (link.doc &&
            docGroupActive &&
            /* 
            melbania: commands docs should not have 'Docs' nav-header link active; without this condition, because their 'sidebar' property is 'docs'(so that they display the left sidebar nav), 'Docs' would be active
            */
            !this.props.current.id.startsWith("commands/")) ||
          /* 
            melbania: commands docs should have 'Commands' nav-header link active
          */
          (link.label === "Commands" &&
    ...
    return (
      <li key={`${link.label}page`} className={itemClasses}>
        <a
          ...
          className={classNames({ tile38NavButton: link.button })} //melbania
        >
